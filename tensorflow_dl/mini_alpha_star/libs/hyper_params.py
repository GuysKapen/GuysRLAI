...
from tensorflow_dl.mini_alpha_star import param
...
if THE_PROJECT_TYPE == ProjectType.AlphaStar:
    Training_Races = AlphaStar_Races
else:
    # Only train Protoss in mini
    Training_Races = MiniStar_Races

DatasetSplitRatio = namedtuple('DataSplitRatio', ['training', 'val', 'test'])
DATASET_SPLIT_RATIO = DatasetSplitRatio(training=0.8, val=.1, test=.1)


# For the action with args to be different with action which has only scalar value
# ArgsAction = namedtyple('ArgsAction', ['action_type', 'delay', 'queue', 'units', 'target_unit', 'target_location'])
...
LabelSize = namedtuple('LabelSize', ['action_type_encoding', 'delay_encoding', 'queue_encoding',
                                     'select_units_encoding', 'target_unit_encoding', 'target_location_encoding'])
...
# note: use mini alpha star which mean use a model needing
# much less parameters and input size through using this, reduce the number of model parameters
...
RLTrainingHyperParameters = namedtuple('RLTrainingHyperParameters', ['learning_rate', 'beta1', 'beta2',
                                                                     'epsilon', 'weight_decay', 'clip', 'seed'])

RL_Training_Hyper_parameters = RLTrainingHyperParameters(learning_rate=1e-5,
                                                         beta1=0,
                                                         beta2=0.99,
                                                         epsilon=1e-5,
                                                         weight_decay=1e-5,
...
StarCraftHyperParameters = namedtuple("StarCraftHyperParameters", ['screen_size', 'world_size', 'max_unit_type',
                                                                   'count_beginning_build_order', 'sc2_default_delay',
                                                                   'max_order_ids', 'max_buffer_ids',
                                                                   'max_add_on_type'])
...
MiniStar_Agent_Interface_Format_Params = AgentInterfaceFormatParams(
    feature_dimensions=sc2_env.Dimensions(screen=64, minimap=32),
    rgb_dimensions=None,
    raw_resolution=None,
    action_space=None,
...
if THE_PROJECT_TYPE == ProjectType.AlphaStar:
    Agent_Interface_Format_Params = AlphaStar_Arch_Hyper_Parameters
else:
