...
from tensorflow_dl.mini_alpha_star.core.sl.dataset import SC2ReplayData, SC2ReplayDataset
...
def train_for_val(feature, replay_data):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for traj in train_loader:
            traj = tf.cast(traj, dtype=tf.float32)

            # with tf.GradientTape()autograd.detect_anomaly():
            ...
            tf.clip_by_norm(parameters, CLIP)
...
def eval(model, criterion, data_loader, train_set, val_set, agent):
    model.eval()
