...
class ScalarEncoder(tf.keras.Model):
    ...
    def __init__(self, n_statistics=10, n_upgrades=SFS.upgrades,
        ...
        self.home_race_fc = layers.Dense(original_32)  # with relu, also goto scalar_context
        self.away_race_fc = layers.Dense(original_32)  # with relu, also goto scalar_context
        self.upgrades_fc = layers.Dense(original_128)  # with relu
        self.enemy_upgrades_fc = layers.Dense(original_128)  # with relu
        self.time_fc = original_64  # a transformer positional encoder

        # additional features
        self.available_actions_fc = layers.Dense(original_64)  # with relu, also goto scalar_context
        self.unit_counts_bow_fc = layers.Dense(original_64)  # A bag-of-words unit count from `entity_list`, with relu
        self.mmr_fc = layers.Dense(original_64)  # mmr is from 0 to 6 (by divison by 1000), with relu
        ...
        self.last_delay_fc = layers.Dense(original_64)  # with relu
        self.last_action_type_fc = layers.Dense(original_128)  # with relu
        self.last_repeat_queued_fc = layers.Dense(original_256)  # with relu

        self.fc_1 = layers.Dense(original_512)  # with relu
        self.fc_2 = layers.Dense(original_512)  # with relu
    ...
    def forward(self, scalar_list):
        ...
        print('torch.log(agent_statistics + 1):', tf.math.log(agent_statistics + 1)) if debug else None
        ...
        if tf.reduce_any(tf.math.is_nan(the_log_statistics)).numpy():
            ...
            if tf.math.is_nan(the_log_statistics).any():
                print('Find NAN the_log_statistics !', the_log_statistics)
                the_log_statistics = tf.ones_like(agent_statistics)

        x = tf.nn.relu(self.statistics_fc(the_log_statistics))
        ...
        x = tf.reshape(x, (x.shape[0], SCHP.count_beginning_build_order * 16))
        print("x:", x) if debug else None
        print("x.shape:", x.shape) if debug else None
        ...
        scalar_context = tf.concat(scalar_context_list, axis=1)
